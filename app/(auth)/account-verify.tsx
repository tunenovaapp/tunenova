import { useResendVerification, useVerify } from "@/api/auth/auth";
  ...
  const handleResend = () => {
    ...
    resendMutate(
      { email: String(email) },
      {
        ...
        onError: (err: any) => {
          setMessage(
            err?.response?.data?.error || "Failed to resend code. Try again."
          );
        },
      }
    );
        ...
        <View style={[styles.inner]}>
          ...
          >
            {isResending ? (
              ...
              <Text
                style={[styles.resendText, timer > 0 && { color: "#6b7280" }]}
              >
                {timer
          ...
          <TouchableOpacity
            ...
            disabled={!code.every(Boolean) || isPending}
            activeOpacity={0.9}
